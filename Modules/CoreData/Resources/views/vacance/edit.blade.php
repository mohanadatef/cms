...
@section('content')
    <section class="content-header">
        <h1>
            Vacance
            <small>Edit Vacance</small>
        </h1>
        <ol class="breadcrumb">
            <li><a href="{{ url('/admin') }}"><i class="fa fa-dashboard"></i>Dashboard</a></li>
            <li><a href="{{ url('/admin/vacance/index') }}"><i class="fa fa-permsissions"></i>Vacance</a></li>
            <li><a href="{{ url('/admin/vacance/edit/'.$data->slug) }}"><i class="fa fa-permsission"></i>Edit Vacance: {{$data->title_en }} / {{$data->title_ar}}</a></li>
        </ol>
    </section>
    <section class="content">
        <div class="box">
            <div class="box-header">
                <h3>Edit Vacance: {{$data->title_en }} / {{$data->title_ar}}</h3>
            </div>
            <!-- /.box-header -->
            <div class="box-body">
                <form id="vacance_edit" action="{{url('admin/vacance/update/'.$data->slug)}}" enctype="multipart/form-data" method="POST">
                    {{csrf_field()}}
                    {{method_field('patch')}}
                    <div class="row">
    <div class="col-md-6">
    ...
    Order : <input type="text" value="{{$data->order}}" class="form-control" name="order" id="order" placeholder="Enter You Order">
                    ...
                    <div align="center">
                        <img src="{{url('public/images/vacance').'/'.$data->image}}" style="width: 100px;height: 100px">
...
@yield('script_description_language')
    {!! JsValidator::formRequest('Modules\CoreData\Http\Requests\admin\Vacance\VacanceEditRequest','#vacance_create') !!}
@endsection