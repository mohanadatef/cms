...
@section('content')
    <section class="content-header">
        ...
        <ol class="breadcrumb">
            ...
            <li><a href="{{ url('/admin/item/create') }}"><i class="fa fa-permsission"></i>Create Item</a></li>
        </ol>
    </section>
    <section class="content">
        <div class="box">
            ...
            <!-- /.box-header -->
            <div class="box-body">
                <form id='item_create' action="{{url('admin/item/store')}}" enctype="multipart/form-data" method="POST">
                    {{csrf_field()}}
                    <div class="form-group{{ $errors->has('product_id') ? ' has-error' : "" }}">
                        Product : <select id="product_id" name="product_id" type="product_id"
                                          class="form-control{{ $errors->has('product_id') ? ' has-error' : "" }}">
                            <option value="" selected disabled>Select</option>
                            @foreach($product as $key => $myproduct)
                                <option value="{{$key}}"> {{$myproduct}}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="row">
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group{{ $errors->has('price') ? ' has-error' : "" }}">
                                Price : <input type="text" value="{{Request::old('price')}}" class="form-control"
                                               name="price" placeholder="Enter You Price">
                            </div>
                        </div>
                        <div class="col-md-6">
...
@yield('script_description_language')
    {!! JsValidator::formRequest('Modules\CoreData\Http\Requests\admin\Item\ItemCreateRequest','#item_create') !!}
@endsection