...
@section('content')
    <section class="content-header">
        <h1>
            Category Service
            <small>Edit Category Service</small>
        </h1>
        <ol class="breadcrumb">
            <li><a href="{{ url('/admin') }}"><i class="fa fa-dashboard"></i>Dashboard</a></li>
            <li><a href="{{ url('/admin/category_service/index') }}"><i class="fa fa-permsissions"></i>Category Service</a></li>
            <li><a href="{{ url('/admin/category_service/edit/'.$data->slug) }}"><i class="fa fa-permsission"></i>Edit Category Service : {{$data->title_en }} / {{$data->title_ar}}</a></li>
        </ol>
    </section>
    <section class="content">
        <div class="box">
            <div class="box-header">
                <h3>Edit Category Service : {{$data->title_en }} / {{$data->title_ar}}</h3>
            </div>
            <!-- /.box-header -->
            <div class="box-body">
                <form id="category_service_edit" action="{{url('admin/category_service/update/'.$data->slug)}}" enctype="multipart/form-data"  method="POST" >
                    {{csrf_field()}}
                    {{method_field('patch')}}
                    <div class="form-group{{ $errors->has('service_id') ? ' has-error' : "" }}">
                        Service :  <select id="service_id" type="service_id" class="form-control{{ $errors->has('service_id') ? ' has-error' : "" }}" name="service_id"  >
                            @foreach($service as $key => $myservice)
                                <option value="{{$key}}"  @if($data->service_id == $key)){ selected  } @else{   }@endif  > {{$myservice}}</option>
    ...
    Order : <input type="text" value="{{$data->order}}" class="form-control" name="order" id="order" placeholder="Enter You Order">
                    ...
                    <div align="center">
                        <img src="{{url('public/images/category_service').'/'.$data->image}}" style="width:300px;height:300px;">
...
@yield('script_description_language')
    {!! JsValidator::formRequest('Modules\CoreData\Http\Requests\admin\Category_Service\Category_ServiceEditRequest','#category_service_edit') !!}
@endsection